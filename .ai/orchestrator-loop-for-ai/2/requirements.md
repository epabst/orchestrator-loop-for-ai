...
### 1.1 Purpose
The purpose of this document is to define the comprehensive requirements for the "Orchestrator Loop for AI," a system designed to automate software development tasks by polling for work (e.g., GitHub issues) and invoking AI agents to execute specific phases of the software development lifecycle (SDLC).
...
The system is intended to run as a CLI-based script, suitable for use in a CI/CD environment or locally on a developer machine (supporting Windows, Mac, and Linux).

## 3. System Features

### 3.1 Work Polling
- The system shall periodically poll configured external sources (e.g., GitHub) for new or updated tasks/issues.
- The system shall indicate when no issues are available for processing.
...
- State transitions shall be defined and managed via a YAML configuration file (`sdlc_config.yaml`) to ensure flexibility and maintainability.
...
### 5.1 Console Output Optimization
- The system shall optimize console output to avoid excessive, repetitive logging when the orchestrator is in a "waiting" state (e.g., waiting for new issues).
- In a "waiting" state, the system shall maintain a single, updating console line to indicate the time elapsed or status of the wait, replacing the generation of new, duplicate log lines for each check interval.
